#!/bin/sh
#
# Manage the minimal Supabase stack (db, auth, storage, meta, studio, kong).
#
# Usage:
#   sh run.sh start | stop | status | logs [service] | pull
#   sh run.sh restart [service] | recreate [service]
#   sh run.sh inspect <service> | printenv <service> | secrets
